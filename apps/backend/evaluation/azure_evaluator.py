...
class AzureRAGEvaluator:
    ...
    async def evaluate_response(
        ...
        coherence_result = await loop.run_in_executor(
            None, lambda: self._coherence_evaluator(query=query, response=response)
        )

        retrieval_result = await loop.run_in_executor(
            None,
            lambda: self._retrieval_evaluator(
                query=query, context=context_str, ground_truth=ground_truth or ""
            ),
        )
        ...
        # Determine pass/fail
        passed = (
            groundedness_score >= self.GROUNDEDNESS_THRESHOLD
            and relevance_score >= self.RELEVANCE_THRESHOLD
            and coherence_score >= self.COHERENCE_THRESHOLD
            and retrieval_score >= self.RETRIEVAL_THRESHOLD
        )

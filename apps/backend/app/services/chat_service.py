...
from app.core.prompts import LLM_UNAVAILABLE_MESSAGE, NOT_FOUND_MESSAGE, RISEN_PROMPT
...
from app.services.entity_ranking import apply_location_boost as _apply_location_boost
...
from app.services.on_your_data_service import OnYourDataResult, OnYourDataService
...
from app.services.query_processor import is_refusal_response as _is_refusal_response
from app.services.query_processor import (
...
from app.services.query_processor import truncate_verbatim as _truncate_verbatim
...
from app.services.search_result import format_rag_context
from app.services.self_reflective_rag import (
    get_self_reflective_service,
)
from app.services.synonym_service import QueryExpansion, get_synonym_service
...
class ChatService:
    ...
    def __init__(
        ...
        if cohere_rerank_service and cohere_rerank_service.is_configured:
            ...
            aoai_key = settings.AOAI_API_KEY
            if aoai_endpoint and aoai_key:
                ...
                logger.info(
                    "Azure OpenAI client initialized for Cohere rerank pipeline"
                )
    ...
    def _reorder_for_attention(
        self, reranked: List[RerankResult]
    ) -> List[RerankResult]:
        """
    ...
    def filter_by_score_window(
        self, reranked: List[RerankResult], query: str, window_threshold: float = 0.6
    ) -> List[RerankResult]:
    ...
    def _calculate_response_confidence(
        self, reranked: List[RerankResult], has_evidence: bool = True
    ) -> Tuple[float, str]:
    ...
    def _boost_confidence_with_grounding(
        ...
        return _boost_confidence_with_grounding_standalone(
            confidence_score, evidence_items, verification
        )

    def _should_return_not_found(
        self, confidence_score: float, confidence_level: str, has_evidence: bool
    ) -> bool:
        """Determine if response should be 'not found' based on confidence."""
        return _should_return_not_found_standalone(
            confidence_score, confidence_level, has_evidence
        )

    # HyDE (Hypothetical Document Embeddings) has been removed.
    # It was disabled for healthcare safety (hallucination vector) and never called.
    # Query variants and RRF are available via query_enhancer module if needed.
    ...
    def _validate_query(
        self, message: str
    ) -> Tuple[Optional[Tuple[str, Optional[Dict]]], Any]:
        """
        Consolidated pre-cache query validation.

        Returns (early_exit, ref_result) where:
        - early_exit is None if query is valid, or a tuple of
          (validation_type, data) for early exit:
          - ("unclear", None)
          - ("clarification", ambiguity_config_dict)
          - ("adversarial", None)
        - ref_result is the policy number detection result (used by callers
          for OData filter building)

        Policy number detection must run first because references like
        "HR-C 05.00" have no vowels and trip the gibberish detector.
        """
        ref_result = _detect_policy_number_standalone(message)

        if not ref_result and self._is_unclear_query(message):
            return ("unclear", None), ref_result

        ambiguity_config = self.detect_device_ambiguity(message)
        if ambiguity_config:
            return ("clarification", ambiguity_config), ref_result

        if self._is_adversarial_query(message):
            return ("adversarial", None), ref_result

        return None, ref_result
    ...
    def _expand_query(self, query: str) -> tuple[str, Optional[QueryExpansion]]:
        ...
        try:
            ...
            if self.cache_service and self.cache_service.enabled:
                self.cache_service.set_expansion(
                    query, expansion.expanded_query, expansion
                )
    ...
    def _should_force_oyd_for_policy_query(
        self, query: str, ref_result: Optional[Tuple[str, str]]
    ) -> bool:
        """
    ...
    async def _handle_instance_search(
        self, search_term: str, policy_id: str
    ) -> ChatResponse:
        """
        ...
        logger.info(
            f"Instance search: term='{search_term}', policy='{policy_id}' -> resolved='{resolved_policy}'"
        )

        # Create service and search
        service = InstanceSearchService(self.search_index.get_search_client())
        result = await asyncio.to_thread(
            service.search_within_policy, policy_ref=resolved_policy, query=search_term
        )
        ...
        for i, instance in enumerate(result.instances[:10], 1):
            page_info = (
                f"Page {instance.page_number}" if instance.page_number else "N/A"
            )
            section_info = ""
            if instance.section:
    ...
    async def process_chat(self, request: ChatRequest) -> ChatResponse:
        ...
        if instance_intent:
            ...
            logger.info(
                f"Instance search query detected: '{search_term}' in policy '{policy_id}'"
            )
            return await self._handle_instance_search(search_term, policy_id)

        # Early query validation (must run before cache lookup)
        validation, ref_result = self._validate_query(request.message)
        if validation:
            vtype, vdata = validation
            logger.info(f"{vtype} query detected: {request.message[:50]}...")
            if vtype == "unclear":
                return ChatResponse(
        ...
        if (
            force_oyd
            and self.on_your_data_service
            and self.on_your_data_service.is_configured
        ):
            logger.info(
                "Policy-number/HR-coded query detected; routing to On Your Data path"
            )
            response = await self._chat_with_on_your_data(request, filter_expr)
            if self.cache_service and self.cache_service.should_cache_response(
                response
            ):
        ...
        if self.cache_service and self.cache_service.enabled:
            cached_response = self.cache_service.get_response(
                request.message, filter_expr
            )
            if cached_response is not None:
                logger.info(
                    f"[CACHE HIT] Response cache hit for: {request.message[:50]}..."
                )
        ...
        if (
            settings.USE_COHERE_RERANK
            and self.cohere_rerank_service
            and self.cohere_rerank_service.is_configured
    ...
    async def process_chat_stream(self, request: ChatRequest):
        ...
        try:
            # Early query validation (shared with non-streaming path)
            validation, ref_result = self._validate_query(request.message)
            if validation:
                vtype, vdata = validation
                if vtype == "unclear":
                    yield sse_event(
                        "answer_chunk",
                        {"type": "answer_chunk", "content": UNCLEAR_QUERY_MESSAGE},
                    )
                    yield sse_event(
                        "metadata",
                        {
                    ...
                    yield sse_event(
                        "clarification",
                        {
                            "type": "clarification",
                            "ambiguous_term": (
                                vdata.get("ambiguous_term", "") if vdata else ""
                            ),
                            "message": vdata.get("message", "") if vdata else "",
                            "options": vdata.get("options", []) if vdata else [],
                    ...
                    yield sse_event(
                        "answer_chunk",
                        {"type": "answer_chunk", "content": adversarial_msg},
                    )
                    yield sse_event(
                        "metadata",
                        {
            ...
            force_oyd = self._should_force_oyd_for_policy_query(
                request.message, ref_result
            )
            if (
                force_oyd
                and self.on_your_data_service
                ...
                yield sse_event(
                    "answer_chunk",
                    {"type": "answer_chunk", "content": oyd_response.response},
                )
                yield sse_event(
                    "evidence",
                    {
                ...
                yield sse_event(
                    "sources", {"type": "sources", "items": oyd_response.sources}
                )
                yield sse_event(
                    "metadata",
                    {
            ...
            if not force_oyd and self._is_out_of_scope_query(request.message):
                ...
                yield sse_event(
                    "answer_chunk",
                    {"type": "answer_chunk", "content": out_of_scope_msg},
                )
                yield sse_event(
                    "metadata",
                    {
            ...
            # Status: Searching
            yield sse_event(
                "status", {"type": "status", "message": "Searching policies..."}
            )
            ...
            if not search_results:
                yield sse_event(
                    "answer_chunk",
                    {"type": "answer_chunk", "content": NOT_FOUND_MESSAGE},
                )
                yield sse_event(
                    "metadata",
                    {
            ...
            # Status: Reranking
            yield sse_event(
                "status", {"type": "status", "message": "Analyzing relevance..."}
            )
            ...
            # Apply score windowing for single-intent queries (matches non-streaming path)
            if reranked and len(reranked) > 3:
                reranked = self.filter_by_score_window(
                    reranked, request.message, window_threshold=0.6
                )
            ...
            # Fall back to OYD if Cohere's top score is too low to be meaningful
            if reranked and reranked[0].cohere_score < 0.10:
                logger.warning(
                    f"[stream] Cohere top score {reranked[0].cohere_score:.3f} below quality threshold; "
                    f"falling back to On Your Data"
                )
                if (
                    self.on_your_data_service
                    and self.on_your_data_service.is_configured
                ):
                    oyd_response = await self._chat_with_on_your_data(
                        request, filter_expr
                    )
                    yield sse_event(
                        "answer_chunk",
                    ...
                    logger.warning(
                        "[stream] OYD fallback unavailable (not configured); proceeding with low-score Cohere results"
                    )
            ...
            # Status: Generating
            yield sse_event(
                "status", {"type": "status", "message": "Generating answer..."}
            )
                ...
                try:
                    ...
                    for chunk in stream:
                        if chunk.choices and chunk.choices[0].delta.content:
                            ...
                            yield sse_event(
                                "answer_chunk",
                                {"type": "answer_chunk", "content": content},
                            )
                    ...
                    if is_not_found:
                        ...
                        # Safety validation (parity with non-streaming path)
                        safety_flags = []
                        try:
                            safety_validator = get_safety_validator(strict_mode=True)
                            safety_result = safety_validator.validate(
                                full_answer, request.message
                            )
                            if not safety_result.is_safe:
                                safety_flags = safety_result.flags
                                logger.warning(f"[stream] Safety flags: {safety_flags}")
                        except Exception as e:
                            logger.warning(f"[stream] Safety validation error: {e}")

                        # Citation verification (parity with non-streaming path)
                        try:
                            citation_verifier = get_citation_verifier()
                            citation_verifier.verify_response(
                                full_answer, evidence_items
                            )
                        except Exception as e:
                            logger.warning(f"[stream] Citation verification error: {e}")
                        ...
                        yield sse_event(
                            "evidence", {"type": "evidence", "items": evidence_items}
                        )
                        yield sse_event(
                            "sources", {"type": "sources", "items": sources}
                        )

                        # Calculate confidence
                        avg_score = sum(r.cohere_score for r in reranked[:3]) / min(
                            3, len(reranked)
                        )
                        confidence = (
                            "high"
                            if avg_score > 0.7
                            else "medium" if avg_score > 0.4 else "low"
                        )
    ...
    async def _chat_with_cohere_rerank(
        self, request: ChatRequest, filter_expr: Optional[str] = None
    ) -> ChatResponse:
        """
        ...
        """
        logger.info(
            f"Using Cohere Rerank pipeline for query: {request.message[:50]}..."
        )
        ...
        policy_routed_query = (
            has_policy_filter
            or self._should_force_oyd_for_policy_query(
                request.message,
        ...
        search_query, forced_refs = self._apply_policy_hints(expanded_query)
        forced_ref_numbers = {
            entry.get("reference") for entry in forced_refs if entry.get("reference")
        }
        ...
        try:
            ...
            if self.cache_service and self.cache_service.enabled:
                ...
                if cached_results is not None:
                    logger.info(
                        f"[CACHE HIT] Search cache hit: {len(cached_results)} results"
                    )
                    search_results = cached_results
            ...
            logger.info(
                f"Retrieved {len(search_results) if search_results else 0} candidates for Cohere reranking"
            )
            ...
            for sr in search_results:
                ...
                if (
                    sr.reference_number in forced_ref_numbers
                    and sr.reference_number not in forced_doc_map
                ):
                ...
                for entry in forced_refs:
                    ...
                    try:
                        ...
                        for sr in targeted:
                            ...
                            if (
                                sr.reference_number
                                and sr.reference_number not in existing_refs
                            ):
                                record = {
                                    "content": sr.content,
                                    "title": sr.title,
                        ...
                        logger.warning(
                            f"Forced reference lookup failed for Ref #{ref}: {e}"
                        )
                original_docs = list(docs_for_rerank)
            ...
            try:
                ...
                corrective_action = crag_service.determine_corrective_action(
                    query=expanded_query, assessments=quality_assessments
                )
                ...
                if corrective_action.action in ("refuse", "proceed_fallback"):
                    ...
                    if corrective_action.relevant_docs:
                        ...
                        logger.info(
                            f"cRAG fallback: using {len(docs_for_rerank)} top-scoring docs"
                        )
                    else:
                        ...
                        logger.warning(
                            f"cRAG refuse: using first {len(docs_for_rerank)} docs"
                        )
                    ...
                    if filtered_docs:
                        ...
                        logger.info(
                            f"cRAG: Filtered to {len(docs_for_rerank)} quality-approved docs"
                        )
                        ...
                        logger.info(
                            "cRAG: using relevant doc indices despite low aggregate score"
                        )
                else:
                ...
                if not docs_for_rerank:
                    logger.info(
                        "cRAG filtering produced no docs; reverting to original candidate set"
                    )
                    docs_for_rerank = original_docs
                    ...
                    if ref and ref not in existing_refs:
                        for candidate in original_docs:
                            if candidate.get("reference_number") == ref:
                                ...
                                logger.info(
                                    f"Forced inclusion of Ref #{ref} to maintain policy coverage"
                                )
                                break

            if not docs_for_rerank:
                logger.warning(
                    "No documents available for reranking after cRAG processing"
                )
                if (
                    self.on_your_data_service
                    and self.on_your_data_service.is_configured
            ...
            logger.info(
                f"Cohere reranked {len(docs_for_rerank)} docs → top {dynamic_top_n} results"
            )
            ...
            if forced_ref_numbers and reranked:
                reranked_refs = {
                    rr.reference_number for rr in reranked if rr.reference_number
                }
                missing_forced = forced_ref_numbers - reranked_refs
                if missing_forced:
                    logger.warning(
                        f"Forced refs {missing_forced} not in reranked results; retrying with min_score=0.05"
                    )
                    reranked_with_lower_threshold = (
                        await self.cohere_rerank_service.rerank_async(
                            query=request.message,
                            documents=docs_for_rerank,
                    ...
                    if reranked_with_lower_threshold:
                        ...
                        retry_refs = {
                            rr.reference_number
                            for rr in reranked_with_lower_threshold
                            if rr.reference_number
                        }
                        found_forced = missing_forced & retry_refs
                        if found_forced:
                            logger.info(
                                f"Sparse fallback found forced refs: {found_forced}"
                            )
            ...
            if not reranked:
                logger.warning(
                    "Cohere rerank returned no results at calibrated threshold; retrying with min_score=0.0"
                )
                reranked = await self.cohere_rerank_service.rerank_async(
                    query=request.message,
                    documents=docs_for_rerank,
            ...
            if reranked and reranked[0].cohere_score < 0.10:
                logger.warning(
                    f"Cohere top score {reranked[0].cohere_score:.3f} below quality threshold; "
                    f"falling back to On Your Data"
                )
                if (
                    self.on_your_data_service
                    ...
                    logger.warning(
                        "OYD fallback unavailable (not configured); proceeding with low-score Cohere results"
                    )
            ...
            if forced_ref_numbers:
                ...
                for rr in reranked:
                    if rr.reference_number in forced_ref_numbers:
                        ...
                        logger.info(
                            f"Boosted forced ref #{rr.reference_number}: {rr.cohere_score:.3f} → {boosted_score:.3f}"
                        )
                    else:
                ...
                reranked = sorted(
                    boosted_reranked, key=lambda r: r.cohere_score, reverse=True
                )
                ...
                for ref in forced_ref_numbers:
                    ...
                    reranked.append(
                        RerankResult(
                            ...
                            page_number=doc.get(
                                "page_number"
                            ),  # Preserve page number for PDF navigation
                            cohere_score=0.35,
            ...
            reranked = _apply_surge_capacity_penalty(
                reranked, penalty=settings.SURGE_CAPACITY_PENALTY
            )
            ...
            if query_entities:
                reranked = _apply_location_boost(
                    reranked, query_entities, boost=settings.LOCATION_MATCH_BOOST
                )
            ...
            if is_multi_policy and len(reranked) > 3:
                ...
                logger.info(
                    f"Applied MMR diversification for multi-policy query: {len(reranked)} diverse results"
                )
            ...
            logger.debug(
                f"Applied attention-aware reordering for {len(reranked)} documents"
            )
            ...
            if self.context_expander and settings.CONTEXT_EXPANSION_ENABLED:
                try:
                    expanded_contexts = await self.context_expander.expand_context(
                        reranked, top_n=settings.CONTEXT_EXPANSION_TOP_N
                    )
                    logger.info(
            ...
            for ec in expanded_contexts:
                content_hash = (
                    hash(ec.original.content[:100]) if ec.original.content else 0
                )
            ...
            if self._is_not_found_response(answer_text):
                ...
                if evidence_items:
                    logger.info(
                        f"NOT_FOUND override: {len(evidence_items)} evidence items exist - trusting evidence"
                    )
            ...
            if not not_found_with_evidence and _is_refusal_response(answer_text):
                logger.info(
                    f"Refusal response detected, clearing {len(evidence_items)} false positive citations"
                )
            ...
            try:
                ...
                verification = citation_verifier.verify_response(
                    response=answer_text, contexts=contexts, sources=sources
                )
                ...
                facts_verified, unverified_facts, fact_flags = (
                    citation_verifier.verify_factual_claims(
                        response=answer_text,
                        contexts=contexts,
                ...
                if not facts_verified and unverified_facts:
                    logger.warning(
                        f"HEALTHCARE SAFETY: Blocking response with unverified facts: {unverified_facts}"
                    )
                    return ChatResponse(
                ...
                refs_verified, fabricated_refs, ref_flags = (
                    citation_verifier.verify_no_fabricated_refs(
                        response=answer_text,
                        context_refs=(
                            verification.context_refs if verification else set()
                        ),
                    )
                )
                ...
                if not refs_verified and fabricated_refs:
                    logger.warning(
                        f"HEALTHCARE SAFETY: Blocking response with fabricated refs: {fabricated_refs}"
                    )
                    return ChatResponse(
            ...
            if safety_result and not safety_result.safe:
                ...
                fallback = (
                    safety_result.fallback_response
                    or f"I could not verify this information against RUSH policies. "
                    f"Please check {settings.POLICYTECH_URL} or contact Policy Administration."
                )
                return ChatResponse(
                    response=fallback,
            ...
            if verification and verification.hallucination_risk > 0.5:
                logger.warning(
                    f"HEALTHCARE SAFETY BLOCK: Hallucination risk {verification.hallucination_risk:.2f}"
                )
                return ChatResponse(
            ...
            needs_review = (
                (safety_result and safety_result.needs_human_review)
                or (verification and verification.hallucination_risk > 0.5)
            ...
            try:
                ...
                critique = self_reflective_service.critique_response(
                    response=answer_text, query=request.message, contexts=contexts
                )

                if not critique.overall_pass:
                    logger.warning(
                        f"Self-Reflective critique failed: {critique.issues}"
                    )
                    ...
                    logger.debug(
                        f"Self-Reflective critique passed: confidence={critique.confidence:.2f}"
                    )
    ...
    def _extract_policy_refs_from_response(self, response_text: str) -> List[dict]:
            ...
            refs.append(
                {
                    ...
                    "reference_number": match.group(2).strip(),
                }
            )
        ...
        ref_num_match = re.search(
            r"Reference\s*Number[:#]?\s*([A-Z0-9\.\-]{2,15})",
            response_text,
            re.IGNORECASE,
        )
        ...
        for match in re.finditer(pattern4, response_text, re.IGNORECASE):
            ...
            if title_before:
                refs.append(
                    {
                        "title": title_before.group(1).strip(),
                        "reference_number": ref_num,
                    }
                )
            else:
    ...
    async def _chat_with_on_your_data(
        self, request: ChatRequest, filter_expr: Optional[str]
    ) -> ChatResponse:
        """
        ...
        """
        logger.info(
            f"Using On Your Data (vectorSemanticHybrid) for query: {request.message[:50]}..."
        )
        ...
        try:
            ...
            if self._is_not_found_response(answer_text):
                logger.warning(
                    f"NOT_FOUND triggered for query: '{request.message[:80]}...'"
                )
                ...
                if not matched_phrase and answer_text == NOT_FOUND_MESSAGE:
                    ...
                    logger.warning(
                        "NOT_FOUND triggered but no phrase matched (empty response?)"
                    )
            ...
            if _is_refusal_response(answer_text):
                logger.info(
                    f"Refusal response detected, clearing {len(result.citations) if result.citations else 0} false positive citations"
                )
                return ChatResponse(
                    response=answer_text,
                    summary=answer_text,
            ...
            if is_multi_policy and len(result.citations) > max_citations:
                ...
                logger.info(
                    f"Applied MMR diversification: {len(result.citations)} -> {len(citations_to_process)} citations"
                )

            # Batch all page_number and metadata lookups in parallel (avoid N+1)
            cits = citations_to_process[:max_citations]

            async def _safe_page_lookup(sf: str, content: str):
                try:
                    return await asyncio.to_thread(
                        self.search_index.lookup_page_number, sf, content
                    )
                except Exception as e:
                    logger.debug(f"Page lookup failed for {sf}: {e}")
                    return None

            async def _safe_metadata_lookup(sf: str):
                try:
                    return await asyncio.to_thread(
                        self.search_index.get_metadata_by_source_file, sf
                    )
                except Exception as e:
                    logger.warning(f"Failed to get metadata for {sf}: {e}")
                    return None

            # Page lookups — one per citation
            page_tasks = [
                (
                    _safe_page_lookup(cit.filepath or "", cit.content)
                    if (cit.filepath and cit.content)
                    else asyncio.sleep(0)
                )
                for cit in cits
            ]

            # Metadata lookups — deduplicate by source_file
            unique_sources = list(dict.fromkeys(c.filepath for c in cits if c.filepath))
            meta_tasks = [_safe_metadata_lookup(sf) for sf in unique_sources]

            all_results = await asyncio.gather(
                *page_tasks, *meta_tasks, return_exceptions=True
            )
            page_results = all_results[: len(page_tasks)]
            meta_results = all_results[len(page_tasks) :]
            metadata_map = {}
            for i, sf in enumerate(unique_sources):
                if not isinstance(meta_results[i], BaseException):
                    metadata_map[sf] = meta_results[i]

            for idx, cit in enumerate(cits):
                source_file = cit.filepath or ""
                raw_page = page_results[idx]
                chunk_page = raw_page if isinstance(raw_page, int) else None
                ...
                if metadata:
                    ...
                    title = _normalize_policy_title(
                        metadata.get("title", "") or cit.title
                    )
                    logger.debug(
                        f"Enriched citation {source_file}: applies_to={applies_to}"
                    )
                else:
                    ...
                    if source_file:
                        ...
                        ref_match = re.search(
                            r"([a-z]{2,4}[-_]?\d{2,4})", source_file.lower()
                        )
                        if ref_match:
                ...
                # Per-citation page_number from content-based lookup
                page_number = cit.page_number
                if page_number is None and chunk_page is not None:
                ...
                if extracted_refs:
                    for ref in extracted_refs[:3]:
                        try:
                            if ref["reference_number"]:
                                ...
                                for r in ref_results:
                                    if r.reference_number and (
                                        r.reference_number == ref["reference_number"]
                                        or ref["reference_number"] in r.reference_number
                                    ):
                                        title = _normalize_policy_title(r.title)
                                        evidence_items.append(
                                            EvidenceItem(
                                                snippet=_truncate_verbatim(
                                                    r.content or ""
                                                ),
                                                citation=r.citation,
                                                title=title,
                                                reference_number=r.reference_number,
                                                policy_number=getattr(
                                                    r, "policy_number", ""
                                                ),
                                                section=r.section,
                            ...
                            logger.warning(
                                f"Supplemental search failed for ref {ref}: {e}"
                            )
            ...
            formatted_summary = _format_answer_with_citations(
                clean_summary, evidence_items
            )
            ...
            logger.warning(
                f"On Your Data failed, falling back to standard retrieval: {e}"
            )
            return await self._chat_with_standard_retrieval(request, filter_expr)
    ...
    async def _chat_with_standard_retrieval(
        ...
        format_rag_context(search_results) if search_results else ""
        evidence_items = (
            build_supporting_evidence(search_results) if search_results else []
        )

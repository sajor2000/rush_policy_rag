...
from app.services.query_processor import normalize_policy_title, truncate_verbatim
...
def extract_quick_answer(response_text: str) -> str:
    ...
    quick_answer = re.sub(
        r"\n*\[Citation:[^\]]+\]", "", quick_answer, flags=re.IGNORECASE
    ).strip()

    # Remove "[Policy Name, Ref #XXX]" citation format
    quick_answer = re.sub(
        r"\s*\[[^\]]*Ref\s*#[^\]]+\][,.]?", "", quick_answer, flags=re.IGNORECASE
    ).strip()

    # Remove "[Reference Number: XXX]" citation format
    quick_answer = re.sub(
        r"\s*\[Reference\s*Number:[^\]]+\][,.]?", "", quick_answer, flags=re.IGNORECASE
    ).strip()
    ...
    quick_answer = re.sub(r"\s*,?\s*\[[^\]]{10,}\][,.]?\s*$", "", quick_answer).strip()

    # Remove trailing "Applies To: SITE." patterns
    quick_answer = re.sub(
        r",?\s*Applies\s*To:\s*[\w,\s\.]+$", "", quick_answer, flags=re.IGNORECASE
    ).strip()
    ...
    quick_answer = re.sub(
        r"\.?\s*Applies\s*To:\s*[☒☐\s\w,\.]+$", "", quick_answer, flags=re.IGNORECASE
    ).strip()

    # Remove "—applies to SITE" or "applies to SITE" at end
    quick_answer = re.sub(
        r"[—\-–]\s*applies to\s+[\w,\s]+\.?$", "", quick_answer, flags=re.IGNORECASE
    ).strip()
...
def format_answer_with_citations(
    answer_text: str, evidence_items: List["EvidenceItem"]
) -> str:
    """
    ...
    for idx, e in enumerate(evidence_items):
        if e.title:
            ...
            normalized = re.sub(
                r"\s+(former|policy|procedure)$", "", normalized, flags=re.IGNORECASE
            )
    ...
    for normalized, info in policy_map.items():
        ...
        if len(title_words) > 2:
            ...
            short_pattern = (
                r"\b"
                + r"\s+(?:and\s+)?".join(re.escape(w) for w in title_words[:3])
                + r"[^.]*?(?=\s*[,.\)]|$)"
            )
        else:
    ...
    if result == answer_text and evidence_items:
        ...
        for idx, e in enumerate(evidence_items[:3]):  # Max 3 citations
            if e.reference_number:
                citations.append(
                    f"**{e.title}** (Ref #{e.reference_number}) [{idx + 1}]"
                )
            else:
...
def build_supporting_evidence(
    ...
    for result in results[:limit]:
        ...
        reference = result.reference_number or extract_reference_identifier(
            result.citation
        )
        ...
        if not source_file:
            ...
            if source_file:
                logger.warning(
                    f"source_file missing for '{result.title}'; derived '{source_file}'"
                )

...
class QueryDecomposer:
    ...
    def __init__(self):
        ...
        self._multi_topic_re = [
            re.compile(p, re.IGNORECASE) for p in self.MULTI_TOPIC_PATTERNS
        ]
        self._conditional_re = [
            re.compile(p, re.IGNORECASE) for p in self.CONDITIONAL_PATTERNS
        ]
    ...
    def _decompose_conditional(self, query: str) -> List[str]:
        ...
        if_match = re.search(
            r"if\s+(.+?),?\s+(?:then\s+)?(?:what|how|does|do)\s+(.+)", query_lower
        )

...
# Common English words that happen to match abbreviations but should NOT be expanded.
# e.g., "it" should NOT become "information technology"
ABBREVIATION_STOPWORDS = frozenset(
    {
        "it",
        "is",
        "in",
...
COMPOUND_EXPANSIONS = {
    ...
    (
        "neonatal",
        "pain",
    ): "Neonatal ICU NICU pain assessment infant newborn FLACC N-PASS",
    ...
    (
        "sedation",
        "policy",
    ): "sedation mechanical ventilation IV sedation moderate procedural",
    ...
    (
        "verbal",
        "order",
    ): "verbal order telephone order VO TO accept receive authorized personnel not authorized scope",
    (
        "telephone",
        "order",
    ): "telephone order verbal order TO VO accept receive authorized personnel not authorized scope",
    (
        "medical",
        "assistant",
    ): "medical assistant MA verbal order telephone order authorized personnel not authorized scope of practice accept receive orders",
    (
        "unit",
        "secretary",
    ): "unit secretary verbal order telephone order authorized personnel not authorized accept receive orders scope",
    (
        "nursing",
        "aide",
    ): "nursing aide CNA certified nursing assistant verbal order telephone order authorized personnel not authorized scope",
    (
        "accept",
        "order",
    ): "accept order receive verbal telephone authorized personnel not authorized RN nurse pharmacist LPN",
    (
        "can",
        "accept",
    ): "accept verbal order telephone order authorized personnel not authorized RN LPN physician pharmacist nurse practitioner",
    (
        "authorized",
        "personnel",
    ): "authorized personnel verbal order telephone order accept receive RN LPN physician pharmacist not authorized",
    # Hand-off communication
    (
        "hand",
        "off",
    ): "handoff hand-off SBAR shift report patient handoff communication transfer",
    (
        "communication",
        "framework",
    ): "SBAR handoff hand-off shift report communication transfer",
    ...
    (
        "research",
        "poster",
    ): "research poster advertisement recruitment IRB approval study flyer participant",
    (
        "research",
        "recruit",
    ): "research recruitment participant enrollment IRB approval advertisement study subject",
    (
        "research",
        "advertisement",
    ): "research advertisement poster recruitment IRB approval study flyer",
    (
        "research",
        "study",
    ): "research study clinical trial IRB protocol participant subject",
    (
        "irb",
        "recruitment",
    ): "IRB recruitment advertisement poster study participant enrollment approval",
    ("irb", "poster"): "IRB poster advertisement recruitment study approval flyer",
    (
        "hang",
        "poster",
    ): "hang poster advertisement recruitment IRB approval study flyer",
    (
        "advertise",
        "study",
    ): "advertise study recruitment poster IRB approval participant",
    # Adverse events
    (
        "adverse",
        "event",
    ): "adverse event AE unanticipated problem safety event incident reporting",
    (
        "adverse",
        "report",
    ): "adverse event report AE reporting safety incident unanticipated problem",
    (
        "health",
        "episode",
    ): "health episode adverse event AE incident safety event unanticipated problem",
    (
        "report",
        "adverse",
    ): "report adverse event AE safety incident unanticipated problem reporting",
    (
        "clinical",
        "trial",
    ): "clinical trial research study IRB protocol adverse event participant",
    # Consent documentation
    ("consent", "sign"): "consent signature sign form documentation witness mark X",
    (
        "consent",
        "signature",
    ): "consent signature sign form documentation witness mark X",
    ("can", "sign"): "sign signature mark X consent form witness documentation",
    (
        "subject",
        "sign",
    ): "subject sign signature consent form mark X witness documentation",
    (
        "research",
        "consent",
    ): "research consent informed consent form signature documentation IRB subject participant",
    (
        "consent",
        "form",
    ): "consent form signature sign mark X witness documentation informed",
    # Patient consent/agreement language (addresses "agree to treatment" → informed consent gap)
    (
        "agree",
        "treatment",
    ): "agree treatment informed consent consent form patient consent authorization procedure consent",
    (
        "patient",
        "agree",
    ): "patient agree informed consent consent form authorization treatment consent patient rights",
    (
        "treatment",
        "consent",
    ): "treatment consent informed consent patient consent authorization surgical consent procedure consent",
    (
        "agree",
        "procedure",
    ): "agree procedure informed consent consent form surgical consent patient authorization",
    (
        "patient",
        "consent",
    ): "patient consent informed consent consent form authorization treatment consent procedure consent",
...
class SynonymService:
    ...
    def _load_synonyms(self):
        ...
        try:
            ...
            self.query_expansion_rules = data.get("query_expansion_rules", {}).get(
                "rules", []
            )
    ...
    def _build_indexes(self):
        ...
        if "medical_abbreviations" in self.synonym_groups:
            ...
            for abbrev, synonyms in mappings.items():
                ...
                self._abbreviations[abbrev.lower()] = (
                    synonyms[0] if synonyms else abbrev
                )
        ...
        if "rush_institution_terms" in self.synonym_groups:
            self._rush_terms = self.synonym_groups["rush_institution_terms"].get(
                "mappings", {}
            )
        ...
        if "software_systems" in self.synonym_groups:
            ...
            for system, synonyms in mappings.items():
                self._abbreviations[system.lower()] = (
                    synonyms[0] if synonyms else system
                )
    ...
    def _apply_compound_expansions(self, query: str, result: QueryExpansion) -> str:
        """
        ...
        for (term1, term2), expansion in COMPOUND_EXPANSIONS.items():
            if self._contains_term(query, term1) and self._contains_term(query, term2):
                result.expansions_applied.append(
                    {"compound": f"{term1}+{term2}", "expansion": expansion}
                )
    ...
    def _apply_single_term_expansions(self, query: str, result: QueryExpansion) -> str:
        """
        ...
        for phrase in sorted(CONTEXT_SPECIFIC_EXPANSIONS.keys(), key=len, reverse=True):
            if len(phrase.split()) > 1 and self._contains_term(query, phrase):
                ...
                new_terms = [
                    w for w in expansion.split() if w.lower() not in query_lower
                ]
                if new_terms:
                    addition = " ".join(new_terms[:4])
                    result.expansions_applied.append(
                        {"context_specific_phrase": phrase, "expansion": addition}
                    )
                    logger.info(
                        f"Context-specific phrase expansion: '{phrase}' → '{addition}' (STOPS HERE)"
                    )
        ...
        for term, expansion in CONTEXT_SPECIFIC_EXPANSIONS.items():
            if len(term.split()) == 1 and self._contains_term(query, term):
                ...
                if new_terms:
                    addition = " ".join(new_terms[:4])
                    result.expansions_applied.append(
                        {"context_specific_term": term, "expansion": addition}
                    )
                    logger.info(
                        f"Context-specific term expansion: '{term}' → '{addition}' (STOPS HERE)"
                    )
                    return f"{query} {addition}"  # STOP after first match
        ...
        for term, expansion in SINGLE_TERM_EXPANSIONS.items():
            if self._contains_term(query, term):
                ...
                if new_terms:
                    addition = " ".join(new_terms[:4])
                    result.expansions_applied.append(
                        {"single_term": term, "expansion": addition}
                    )
        ...
        for term, expansion in NEUTRAL_FALLBACKS.items():
            if self._contains_term(query, term):
                ...
                if new_terms:
                    addition = " ".join(new_terms[:4])
                    result.expansions_applied.append(
                        {"neutral_fallback": term, "expansion": addition}
                    )
                    logger.info(
                        f"Neutral fallback expansion: '{term}' → '{addition}' (NEUTRAL, STOPS CASCADE)"
                    )
    ...
    def expand_query(
        self, query: str, max_expansions: int = 3, max_expansion_ratio: float = 2.0
    ) -> QueryExpansion:
        """
        ...
        """
        result = QueryExpansion(original_query=query, expanded_query=query)
        ...
        for word in words:
            ...
            if (
                word_lower in self._abbreviations
                and word_lower not in ABBREVIATION_STOPWORDS
            ):
                expansion = self._abbreviations[word_lower]
                result.abbreviations_expanded.append(
                    {"abbreviation": word, "expansion": expansion}
                )
        ...
        expanded_query = self._add_context_for_short_queries(
            query, expanded_query, result
        )

        # 6. Apply compound term expansions (NICU + pain -> neonatal terms)
        expanded_query, compound_matched = self._apply_compound_expansions(
        ...
        if len(expanded_words_final) > max_words:
            ...
            logger.info(
                f"Truncated expansion: {len(expanded_words_final)} -> {max_words} words"
            )
        ...
        if (
            result.expansions_applied
            or result.misspellings_corrected
            or result.abbreviations_expanded
        ):
    ...
    def _add_context_for_short_queries(
        self, original: str, current: str, result: QueryExpansion
    ) -> str:
        """
        ...
        for word in words:
            ...
            if word_lower in context_map:
                ...
                result.expansions_applied.append(
                    {"term": word, "context_added": context_terms}
                )
        ...
        for rule in self.query_expansion_rules:
            ...
            try:
                if re.match(pattern, original, re.IGNORECASE):
                    ...
                    result.expansions_applied.append(
                        {"pattern": pattern, "additions": expand_with[:2]}
                    )
    ...
    def _expand_multiword_terms(
        ...
        if "hospital_codes" in self.synonym_groups:
            for code, synonyms in (
                self.synonym_groups["hospital_codes"].get("mappings", {}).items()
            ):
                if code.lower() in original_lower:
                    ...
                    if synonyms:
                        ...
                        result.expansions_applied.append(
                            {"term": code, "expansion": synonyms[0]}
                        )
                    break

        # Department/unit names
        if "departments_units" in self.synonym_groups:
            for dept, synonyms in (
                self.synonym_groups["departments_units"].get("mappings", {}).items()
            ):
                if dept.lower() in original_lower:
                    # Add abbreviation if available
                    abbrev = next(
                        (s for s in synonyms if s.isupper() and len(s) <= 4), None
                    )
                    if abbrev:
                        expanded = f"{expanded} {abbrev}"
                        result.expansions_applied.append(
                            {"term": dept, "expansion": abbrev}
                        )
                    break
        ...
        for term, expansion in communication_terms.items():
            if term in original_lower:
                ...
                result.expansions_applied.append({"term": term, "expansion": expansion})
    ...
    def get_synonyms_for_term(
        self, term: str, category: Optional[str] = None
    ) -> List[str]:

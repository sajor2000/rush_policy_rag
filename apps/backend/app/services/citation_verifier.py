...
class CitationVerifier:
    ...
    def verify_response(
        self, response: str, contexts: List[str], sources: List[Dict[str, Any]]
    ) -> VerificationResult:
        """
        ...
        if ungrounded_claims:
            ...
            logger.warning(
                f"High-risk claims not found in context: {ungrounded_claims}"
            )
    ...
    def verify_factual_claims(
        self, response: str, contexts: List[str], is_multi_policy: bool = False
    ) -> tuple:
        """
        ...
        for pattern in self.EXACT_MATCH_PATTERNS:
            ...
            for match in matches:
                ...
                if (
                    match_lower in combined_context
                    or normalized_match in normalized_combined
                ):
                    found = True
                ...
                if not found and is_multi_policy:
                    for ctx in contexts:
                        ...
                        if (
                            match_lower in ctx_lower
                            or normalized_match in normalized_ctx
                        ):
                            found = True
                            break
        ...
        if unverified_claims:
            ...
            if is_multi_policy and len(unverified_claims) <= 2:
                ...
                logger.info(
                    f"Multi-policy query: {len(unverified_claims)} minor unverified claims (allowed)"
                )
    ...
    def verify_no_fabricated_refs(self, response: str, context_refs: Set[str]) -> tuple:
        """
        ...
        for ref in cited_refs:
            if (
                ref not in normalized_context_refs
                and ref.lstrip("0") not in normalized_context_refs
            ):
                fabricated.add(ref)

        if fabricated:
            logger.warning(
                f"HEALTHCARE SAFETY: Fabricated policy refs detected: {fabricated}"
            )

...
class SelfReflectiveRAGService:
    ...
    def critique_response(
        self, response: str, query: str, contexts: List[str]
    ) -> CritiqueResult:
        """
        ...
        if not is_relevant:
            issues.append(
                f"Low query-response relevance: {term_overlap:.1%} term overlap"
            )
        ...
        grounding_term_count = sum(
            1 for term in self.HEALTHCARE_GROUNDING_TERMS if term in response_lower
        )
        is_grounded = has_grounding and grounding_term_count >= 2

        if grounding_term_count < 2:
            issues.append(
                f"Low healthcare grounding: only {grounding_term_count} grounding terms"
            )
        ...
        if unsupported_claims:
            issues.append(
                f"Potentially unsupported generalizations: {unsupported_claims[:3]}"
            )
        ...
        confidence = self._calculate_confidence(
            ...
            is_grounded,
            term_overlap,
            grounding_term_count,
            len(unsupported_claims),
            len(speculation_found),
        )

        # Overall pass requires all checks
        overall_pass = (
            is_relevant and is_supported and is_grounded and confidence >= 0.6
        )

...
def diagnose(ref_number: str):
    ...
    if not results:
        ...
        print("  → This policy may not be indexed at all.")
        print("  → Action: Run full_pipeline_ingest.py\n")
        return False
    ...
    for r in results:
        ...
        print(
            f"  chunk[{idx:2d}]  page={page_str:>4s}  level={level:<10s} "
    ...
    if not pages_seen or max(pages_seen, default=0) <= 1:
        ...
        if null_page_count > 0:
            print("  → Fallback chunking produced chunks WITHOUT page numbers.")
            print("  → The code fix (chunker.py) is applied. RE-INDEX REQUIRED.")
        else:
            print("  → Page 2+ chunks may be missing from ingestion.")
            print("  → RE-INDEX REQUIRED.")
        needs_reindex = True
    elif null_page_count > 0:
        print(f"\n  PARTIAL: {null_page_count} chunks have null page_number.")
        print("  → RE-INDEX RECOMMENDED to populate page numbers.")
        needs_reindex = True
    else:
        print("\n  OK: Multi-page chunks exist with page numbers.")
        print("  → Bug 2 filter fix should resolve retrieval ranking.")

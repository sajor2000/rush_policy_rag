...
class TestRateLimitBypass:
    ...
    def test_xff_spoofing_different_ips(self):
        ...
        assert ip1 != ip2, "FINDING: Rate limit can be bypassed by rotating X-Forwarded-For"
    ...
    def test_xff_chain_rightmost_ip_extracted(self):
        ...
        assert ip == "10.0.0.1", "Should extract second-to-last IP from XFF chain (rightmost client)"
    ...
    def test_no_proxy_headers_uses_remote_addr(self):
        ...
        with patch("app.core.rate_limit.get_remote_address", return_value="192.168.1.100"):
            ip = get_real_client_ip(req)
...
class TestAuthBypass:
    ...
    def test_auth_enabled_by_default(self):
        ...
        assert settings.REQUIRE_AAD_AUTH is True, \
    ...
    async def test_auth_enabled_rejects_malformed_bearer(self):
        ...
        with patch("app.dependencies.settings") as mock_settings, \
             patch("app.dependencies._get_auth_validator") as mock_get_validator:

            mock_settings.REQUIRE_AAD_AUTH = True

            # Create a mock validator that raises TokenValidationError
            mock_validator = MagicMock()
            mock_validator.validate.side_effect = TokenValidationError("Invalid token format")
...
class TestHealthEndpointDisclosure:
    ...
    def test_health_exposes_architecture_details(self, client):
        ...
        if response.status_code == 200:
            data = response.json()
            exposed_keys = {"search_index", "on_your_data", "circuit_breakers", "blob_storage"}
...
class TestCORSEnforcement:
    ...
    def test_disallowed_origin_no_cors(self, client):
        ...
        allow_origin = response.headers.get("access-control-allow-origin", "")
        assert "evil-attacker" not in allow_origin, \
    ...
    def test_delete_method_not_allowed(self, client):
        ...
        assert "DELETE" not in allow_methods.upper(), \
            "DELETE method should not be allowed via CORS"
...
class TestErrorDisclosure:
    ...
    def test_chat_500_returns_generic_message(self, client):
        ...
        try:
            ...
            with patch("app.services.chat_service.ChatService.process_chat") as mock_process:
                mock_process.side_effect = Exception("Internal DB error with connection string xyz")

                response = client.post(
                    "/api/chat",
                    json={"message": "What is the hand hygiene policy?"}
                )
                if response.status_code == 500:
                    detail = response.json().get("detail", "")
                    assert "connection string" not in detail.lower(), \

...
test_cases = [
    ...
    # Multiple locations
    ("What is the policy at the bedside during a procedure?", "What is the policy?"),
    # Location in middle
    ("Can I use my phone in a patient room for work?", "Can I use my phone for work?"),
    # Possessive form
    (
        "What is the patient room's cleaning policy?",
        "What is the patient room's cleaning policy?",
    ),  # Should NOT strip "patient room's"
    # Plural with "for" - NOT stripped (could be specific scope, not just location)
    ("What is the policy for patient rooms?", "What is the policy for patient rooms?"),
    # With comma and no space
    ("At the bedside,what should I do?", "what should I do?"),
    # Multiple commas
    (
    ...
    # No location context
    ("What is the HIPAA policy?", "What is the HIPAA policy?"),
    # Department codes should NOT be stripped
    ("What is the ED policy for triage?", "What is the ED policy for triage?"),
    # Entity names should NOT be stripped
    ("What is the policy at Oak Park?", "What is the policy at Oak Park?"),
    ...
    ("in a patient room", ""),  # Edge case: empty query after normalization
    # Unicode and special chars
    ("What is the policy in a patient room?", "What is the policy?"),
    # Case variations
    ("In A Patient Room, what is the policy?", "what is the policy?"),
]
...
def run_tests():
    ...
    for i, (input_query, expected) in enumerate(test_cases, 1):
        current_result, current_context = normalize_location_context_current(
            input_query
        )
        ...
        if has_bug:
            print(
                f"  Fixed:    '{fixed_result}' {' ✓' if fixed_result == expected else ' ❌ STILL BROKEN'}"
            )
            bugs_found.append(
                {
                    "test_num": i,

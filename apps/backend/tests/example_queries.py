...
from policy_rag_setup import get_credential, load_risen_prompt, query_knowledge_base
...
def main():
    ...
    agent_client = KnowledgeAgentRetrievalClient(
        endpoint=SEARCH_ENDPOINT, agent_name=KNOWLEDGE_BASE_NAME, credential=credential
    )
    ...
    for i, query in enumerate(example_queries, 1):
        ...
        try:
            query_knowledge_base(
                agent_client, user_message=query, system_message=SYSTEM_MESSAGE
            )
    ...
    while True:
        try:
            ...
            query_knowledge_base(
                agent_client, user_message=user_input, system_message=SYSTEM_MESSAGE
            )
